...
import { NoticeFormService, NoticeFormGroup } from './notice-form.service';
...
export class NoticeUpdateComponent implements OnInit {
  ...
  editForm: NoticeFormGroup = this.noticeFormService.createNoticeFormGroup();

  constructor(
    protected dataUtils: DataUtils,
    protected eventManager: EventManager,
    protected noticeService: NoticeService,
    protected noticeFormService: NoticeFormService,
  ...
  setFileData(event: Event, field: string, isImage: boolean): void {
    this.dataUtils.loadFileToForm(event, this.editForm, field, isImage).subscribe({
      error: (err: FileLoadError) =>
        this.eventManager.broadcast(new EventWithContent<AlertError>('eStudentHubApp.error', { message: err.message })),
    });
  }
  ...
  save(): void {
    this.isSaving = true;
    const notice = this.noticeFormService.getNotice(this.editForm);
    if (notice.id !== null) {
  ...
  protected updateForm(notice: INotice): void {
    this.notice = notice;
    this.noticeFormService.resetForm(this.editForm, notice);
  }
}
